...
const silhouetteOutPath = path.join(
  root,
  "client",
  "components",
  "icons",
...
// Union of both color masks — used for the single-color silhouette variant
// (decorative watermark use, where a two-tone mark can't be tinted/faded as
// one color the way the old currentColor-based LogoMark could).
async function buildSilhouetteMask(raw, info) {
  const { width, height, channels } = info;
  const mask = Buffer.alloc(width * height);
  for (let i = 0; i < width * height; i++) {
    const o = i * channels;
...
async function main() {
  ...
  const [navyMask, copperMask, silhouetteMask] = await Promise.all([
    buildMask(raw, info, "navy"),
    buildMask(raw, info, "copper"),
    buildSilhouetteMask(raw, info),
  ]);

  const [navyPath, copperPath, silhouettePath] = await Promise.all([
    traceLayer(navyMask, INK_900),
    traceLayer(copperMask, COPPER_600),
    traceLayer(silhouetteMask, "currentColor"),
  ]);
...
`;
  ...
  const silhouetteD = silhouettePath.match(/\sd="([^"]+)"/)?.[1];
  if (!silhouetteD) throw new Error("Could not extract path data from silhouette trace");

  const component = `interface LogoSilhouetteProps {
  size?: number;
  className?: string;
}

// Auto-generated by scripts/vectorize-logo.mjs from public/new_logo.png — do
// not hand-edit; rerun the script instead.
export default function LogoSilhouette({
  size = 180,
  className,
}: LogoSilhouetteProps) {
  return (
    <svg
      ...
      className={className}
    >
      <path d="${silhouetteD}" fill="currentColor" />
    </svg>
  );
}
`;

  await writeFile(silhouetteOutPath, component);
  console.log(`Silhouette component written to ${path.relative(root, silhouetteOutPath)}`);
}
